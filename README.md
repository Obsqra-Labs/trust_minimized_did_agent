# MatchAI MCP Agent — Receipt Verification (Stwo/LuminAIR ready)

This folder is a standalone repo scaffold for a governed MCP agent that proves receipt integrity. It pairs your existing FastAPI gateway (emits signed receipts + Starknet anchors) with a Stwo/LuminAIR-ready receipt-verification circuit and a simple host CLI.

It is intentionally generic: a DID-enabled agent with trust-minimized tool calls, signed/anchored receipts, and a proof-ready payload. No external partnerships are assumed. Future hardening can add on-chain constraints or ZK guardrails for L1/L2; today the receipts are verified locally and can be anchored to L2.

## What this shows
- Agent (MCP host shim) calls governed tools via your gateway (payments/retrieval).
...
- Optional on-chain anchor: reuse existing Starknet Sepolia contract to bind receipt hashes; Voyager links.
...
1) Run the gateway (your verified MCP gateway on :4005).
2) From `scripts/`, run:
   ```bash
...
4) Proofs are local-only; on-chain verifier is not required. You can still anchor the receipt hash to L2 for auditability.
...
- DID-enabled, trust-minimized tool calls against a verified MCP gateway.
